...
def compute_near_field(simulation=None, X=None, Y=None, Z=None, type='scatt',
    ...
    """Return a list of two elements with the electric and magnetic near field components of a certain type.

    Args:
        simulation (smuthi.simulation.Simulation):  Simulation object
        X, Y, Z (float or np.arrays):               Near field query points
        Poyting (boolean):                          Return also a third element containing the cross(E,conj(H))
        type (string):                              a non-ambiguous abbreviation of either 'scattered',
                                                    'initial' or 'internal'
    ...
    e_x = np.zeros_like(X, dtype=np.complex64) # I think that at the moment the output we get
    e_y = np.zeros_like(X, dtype=np.complex64) # from the electric_field and magnetic_field
    e_z = np.zeros_like(X, dtype=np.complex64) # routines is only this much precise
    h_x = np.zeros_like(X, dtype=np.complex64)
    h_y = np.zeros_like(X, dtype=np.complex64)
    h_z = np.zeros_like(X, dtype=np.complex64)
    ...
    if 'int' in type:
        int_fld_exp = intf.internal_field_piecewise_expansion(simulation.initial_field.vacuum_wavelength,
                                                              simulation.particle_list,
                                                              simulation.layer_system)
    ...
    for s in tqdm(range(X.shape[0]), desc=descr.ljust(26), file=sys.stdout,
        ...
        if 'sca' in type:
            ...
            h_x[s,], h_y[s,], h_z[s,] = scat_fld_exp.magnetic_field(xarr, yarr, zarr, simulation.initial_field.vacuum_wavelength)
        elif 'ini' in type:
            e_x[s,], e_y[s,], e_z[s,] = simulation.initial_field.electric_field(xarr, yarr, zarr, simulation.layer_system)
            h_x[s,], h_y[s,], h_z[s,] = simulation.initial_field.magnetic_field(xarr, yarr, zarr, simulation.layer_system)
        elif 'int' in type:
            e_x[s,], e_y[s,], e_z[s,] = int_fld_exp.electric_field(xarr, yarr, zarr)
            h_x[s,], h_y[s,], h_z[s,] = int_fld_exp.magnetic_field(xarr, yarr, zarr, simulation.initial_field.vacuum_wavelength)

    E = np.stack((e_x, e_y, e_z))
    H = np.stack((h_x, h_y, h_z))
...
def show_near_field(simulation=None, quantities_to_plot=None,
    ...
    """Compute and save the electric and magnetic near field values. If the queried domain represents a 2D plane (i.e.,
    the xy-plane if zmin=zmax, etc.), near field data can be also plotted and the relative figures saved to image files.

    Args:
        simulation (smuthi.simulation.Simulation):  Simulation object
        quantities_to_plot:     List of strings that specify what to plot.
                                ...
                                and analogue strings for the magnetic field replacing 'E' with 'H'
    ...
    # let's compute both E and H, assuming that in the future the effort to get both will be comparable to getting only one of the two
    e_scat, h_scat = compute_near_field(simulation=simulation, X=xarr, Y=yarr, Z=zarr, type='scatt.')
    e_init, h_init = compute_near_field(simulation=simulation, X=xarr, Y=yarr, Z=zarr, type='initl.')
    if show_internal_field:
        e_int, h_int = compute_near_field(simulation=simulation, X=xarr, Y=yarr, Z=zarr, type='intrn.')
    ...
    for quantity in quantities_to_plot:
        fieldtype = 'E' if ('E' in quantity) else 'H'
        f_scat = np.squeeze(e_scat) if fieldtype == 'E' else np.squeeze(h_scat)
        f_init = np.squeeze(e_init) if fieldtype == 'E' else np.squeeze(h_init)
        if show_internal_field:
            f_int  = np.squeeze(e_int)  if fieldtype == 'E' else np.squeeze(h_int)
        ...
        for show_opt, save_opt in zip(cycle(show_opts), save_opts) if len(show_opts) < len(save_opts) else zip(show_opts, cycle(save_opts)):
            ...
            try:
                ...
                print("Figures can be shown/saved only for 2D plotting domains. Saving fields to HDF5 is still allowed, though.")
    ...
    if save_data:
        ...
        if data_format.lower() == 'hdf5':
            ...
            with h5py.File(outputdir + '/data.hdf5', 'a') as f:
                ...
                g.create_dataset('init_electric_field', data=e_init, dtype='c16', compression="gzip")
                g.create_dataset('init_magnetic_field', data=h_init, dtype='c16', compression="gzip")
                g.create_dataset('scat_electric_field', data=e_scat, dtype='c16', compression="gzip")
                g.create_dataset('scat_magnetic_field', data=h_scat, dtype='c16', compression="gzip")
                if show_internal_field:
                    g.create_dataset('inte_electric_field', data=e_int, dtype='c16', compression="gzip")
                    g.create_dataset('inte_magnetic_field', data=h_int, dtype='c16', compression="gzip")
            ...
            np.savetxt(outputdir + '/e_init_x.dat', e_init[0,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/e_init_y.dat', e_init[1,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/e_init_z.dat', e_init[2,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/e_scat_x.dat', e_scat[0,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/e_scat_y.dat', e_scat[1,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/e_scat_z.dat', e_scat[2,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/h_init_x.dat', h_init[0,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/h_init_y.dat', h_init[1,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/h_init_z.dat', h_init[2,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/h_scat_x.dat', h_scat[0,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/h_scat_y.dat', h_scat[1,], fmt=fmt, delimiter=',')
            np.savetxt(outputdir + '/h_scat_z.dat', h_scat[2,], fmt=fmt, delimiter=',')
            if show_internal_field:
                np.savetxt(outputdir + '/e_int_x.dat', e_int[0,], fmt=fmt, delimiter=',')
                np.savetxt(outputdir + '/e_int_y.dat', e_int[1,], fmt=fmt, delimiter=',')
                np.savetxt(outputdir + '/e_int_z.dat', e_int[2,], fmt=fmt, delimiter=',')
                np.savetxt(outputdir + '/h_int_x.dat', h_int[0,], fmt=fmt, delimiter=',')
                np.savetxt(outputdir + '/h_int_y.dat', h_int[1,], fmt=fmt, delimiter=',')
                np.savetxt(outputdir + '/h_int_z.dat', h_int[2,], fmt=fmt, delimiter=',')
        else:
