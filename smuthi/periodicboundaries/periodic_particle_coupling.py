...
import smuthi.periodicboundaries.ewald_lattice_sums as els
...
def periodic_coupling_matrix(vacuum_wavelength, k0t, azimuthal_angle, layer_thicknesses, layer_refractive_indices,
    ...
    shape = chf.index_block(s_max - 1, lmax_array, mmax_array)[1]    
    ...
    for s1 in prange(s_max):
        idx1 = chf.index_block(s1, lmax_array, mmax_array)
        for s2 in prange(s_max):
            idx2 = chf.index_block(s2, lmax_array, mmax_array)
        ...
        for tau in range(2):
            for m in range(-m_max1, m_max1 + 1):
                for l in range(max(1, abs(m)), l_max1 + 1):
                    ...
                    m_vec[0][n] = m
                    for pol in range(2):
                        B[0][pol, n, :] = chf.transformation_coefficients_vwf(tau, l, m, pol, kp=k_parallel, kz=kz,
        ...
        for tau in range(2):
            for m in range(-m_max2, m_max2 + 1):
                for l in range(max(1, abs(m)), l_max2 + 1):
                    ...
                    m_vec[1][n] = m
                    for pol in range(2):
                        B[1][pol, n, :] = chf.transformation_coefficients_vwf(tau, l, m, pol, kp=k_parallel, kz=kz,
...
def periodic_layer_mediated_coupling_block(vacuum_wavelength, layer_thicknesses, layer_refractive_indices,
    ...
    for pol in range(2):
        L[pol, :, :, :] = chf.layersystem_response_matrix(pol, layer_thicknesses, layer_refractive_indices,
        ...
        for m in range(-m_max1, m_max1 + 1):
            for l in range(max(1, abs(m)), l_max1 + 1):
                ...
                for iplmn in range(2):
                    for pol in range(2):
                        B[0][pol, iplmn, n, :] = chf.transformation_coefficients_vwf(tau, l, m, pol, kp=k_parallel, kz=kz,
                                                                                 pilm=pilm[iplmn],
    ...
    for tau in range(2):
        for m in range(-m_max2, m_max2 + 1):
            for l in range(max(1, abs(m)), l_max2 + 1):
                ...
                for iplmn in range(2):
                    for pol in range(2):
                        B[1][pol, iplmn, n, :] = chf.transformation_coefficients_vwf(tau, l, m, pol, kp=k_parallel, kz=kz,
                                                                                 pilm=pilm[iplmn],
