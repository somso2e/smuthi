...
class TestNearFieldCUDA(unittest.TestCase):
    def testElectricField(self):
        ...
        h_x_scat_cpu, h_y_scat_cpu, h_z_scat_cpu = scat_fld_exp.magnetic_field(xarr, yarr, zarr, ld)
        h_x_init_cpu, h_y_init_cpu, h_z_init_cpu = simulation.initial_field.magnetic_field(xarr, yarr, zarr, lay_sys)

        cu.enable_gpu()
        scat_fld_exp = sf.scattered_field_piecewise_expansion(ld, part_list, lay_sys)
        h_x_scat_gpu, h_y_scat_gpu, h_z_scat_gpu = scat_fld_exp.magnetic_field(xarr, yarr, zarr, ld)
        h_x_init_gpu, h_y_init_gpu, h_z_init_gpu = simulation.initial_field.magnetic_field(xarr, yarr, zarr, lay_sys)

        np.testing.assert_allclose(np.linalg.norm(h_x_scat_cpu), np.linalg.norm(h_x_scat_gpu), rtol=1e-5)
        np.testing.assert_allclose(np.linalg.norm(h_y_scat_cpu), np.linalg.norm(h_y_scat_gpu), rtol=1e-5)
        np.testing.assert_allclose(np.linalg.norm(h_z_scat_cpu), np.linalg.norm(h_z_scat_gpu), rtol=1e-5)
        np.testing.assert_allclose(np.linalg.norm(h_x_init_cpu), np.linalg.norm(h_x_init_gpu), rtol=1e-5)
        np.testing.assert_allclose(np.linalg.norm(h_y_init_cpu), np.linalg.norm(h_y_init_gpu), rtol=1e-5)
        np.testing.assert_allclose(np.linalg.norm(h_z_init_cpu), np.linalg.norm(h_z_init_gpu), rtol=1e-5)
