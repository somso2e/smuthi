...
def show_near_field(quantities_to_plot=None, save_plots=False, show_plots=True, save_animations=False, save_data=False,
    ...
    if xmin == xmax:
        dim1vec = np.arange(ymin, ymax + resolution_step, resolution_step)
        dim2vec = np.arange(zmin, zmax + resolution_step, resolution_step)
        yarr, zarr = np.meshgrid(dim1vec, dim2vec)
        xarr = yarr - yarr + xmin
        dim1name = 'y (' + simulation.length_unit + ')'
        dim2name = 'z (' + simulation.length_unit + ')'
    elif ymin == ymax:
        dim1vec = np.arange(xmin, xmax + resolution_step, resolution_step)
        dim2vec = np.arange(zmin, zmax + resolution_step, resolution_step)
        ...
        dim1vec = np.arange(xmin, xmax + resolution_step, resolution_step)
        dim2vec = np.arange(ymin, ymax + resolution_step, resolution_step)
    ...
    if interpolate_step is None:
        ...
        dim1vecfine = np.arange(dim1vec[0], dim1vec[-1] + interpolate_step, interpolate_step)
        dim2vecfine = np.arange(dim2vec[0], dim2vec[-1] + interpolate_step, interpolate_step)
